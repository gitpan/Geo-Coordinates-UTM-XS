...
PPCODE:
    {
        ...
        letter = latitude_letter[(int)(10 + latitude_deg / 8)];
        ...
        longitude_origin = deg2rad * ((zone - 1) * 6 - 180 + 3);
        ...
        A = cos_latitude * (longitude - longitude_origin);
        ...
        XPUSHs(sv_2mortal(newSVpvf("%d%c", zone, letter)));
...
PPCODE:
    {
        ...
        zone_pv = SvPV(zone, zone_len);
        zone_letter = zone_pv[zone_len - 1];
        if ((zone_len >=2 && zone_len <= 3) &&
            (zone_pv[0] >= '0' && zone_pv[0] <= '9') &&
            (zone_len == 2 || (zone_pv[1] >= '0' && zone_pv[1] <= '9')) &&
            strchr(latitude_letter, zone_letter)) {
        ...
        phi1 = mu + (1.5 * e1 - e1_3 * (27./32.)) * sin(2 * mu) + (e1_2 * (21./16.) - e1_4 * (55./32.)) * sin(4 * mu) + (151 * e1_3 * (151./96.)) * sin(6 * mu);
